 ...
 * Measures may be verbose and may not be of interest when only the {@linkplain QualityElement#getQualityResults() result}
 * of data quality measures is desired. For allowing more compact {@link QualityElement}s, ISO 19157 does not store
 * {@code Measure} instance directly into {@link QualityElement}, but instead stores {@link MeasureReference} which
 * can be used for fetching full {@link QualityMeasure} description from a measure register or catalogue if desired.
 *
 * <p>GeoAPI extends the ISO 19157 model by allowing {@link QualityElement} to provide directly a {@link QualityMeasure} instance.
 ...
 * connecting themselves to a catalogue when {@link QualityElement#getMeasure()} is first invoked.</p>
 *
 ...
 * @see QualityElement#getMeasure()
 *
 * @since 3.1
 */
@UML(identifier="QualityMeasure", specification=ISO_19157)
public interface QualityMeasure {
    ...
    default Collection<? extends MeasureDescription> getExamples() {
        return Collections.emptyList();
    }
}
