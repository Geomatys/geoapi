...
public final class ReferenceSystemTypeTest implements InvocationHandler {
    ...
    /**
     * If the name contains a "2D" or "3D" part, then it shall match the declared number of dimensions.
     *
     * @throws ReflectiveOperationException if an error occurred while accessing private fields.
     */
    @Test
    public void verifyDimensionOfFirst() throws ReflectiveOperationException {
        final Field dimField = ReferenceSystemType.class.getDeclaredField("dimOfFirst");
        dimField.setAccessible(true);
        for (ReferenceSystemType type : ReferenceSystemType.values()) {
            final String name = type.name();
            final int dimOfFirst = dimField.getInt(type);
            if (name.contains("3D")) assertEquals(3, dimOfFirst, name);
            if (name.contains("2D")) assertEquals(2, dimOfFirst, name);
            if (name.contains("VERTICAL")) {
                assertTrue(dimOfFirst == 0 || dimOfFirst == 2, name);
            }
        }
    }
    ...
    public void testIsInstance() {
        ...
        for (final ReferenceSystemType type : filteredValues) {
            ...
            for (ReferenceSystemType candidate : values) {
                assertEquals(candidate == type, candidate.isInstance(mock));
            }
        }
    }
